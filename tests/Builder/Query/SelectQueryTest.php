...
namespace Somnambulist\Components\QueryBuilder\Tests\Builder\Query;
...
class SelectQueryTest extends TestCase
{
    ...
    public function testSelectFieldsFromTable(): void
    {
        ...
        $query->resetClause('select', 'from', 'order')->select('name')->from('authors')->orderBy(['name' => 'DESC']);
    ...
    public function testSelectWithJoins(): void
    {
        ...
        $query->resetClause('join')->join('authors', on: $query->newExpr()->equalFields('author_id', 'authors.id'));
    ...
    public function testSelectOrderByAssociativeArrayContainingExtraExpressions(): void
    {
        ...
        $this->expectExceptionMessage(
            ...
            'Use "Somnambulist\Components\QueryBuilder\Builder\Expressions\QueryExpression" or numeric array instead.'
        );
    ...
    public function testSelectOrderByComplexExpression(): void
    {
        ...
        $query->select(['id'])
              ->from('articles')
              ->orderBy(function (QueryExpression $exp, AbstractQuery $query) {
                  return $exp
                      ->case()
                      ->when(['author_id' => 1])
                      ->then(1)
    ...
    public function testSelectHaving(): void
    {
        ...
        $query->resetClause('having')->having(['count(author_id)' => 2], ['count(author_id)' => 'integer']);

        $sql = $this->compiler->compile($query, new ValueBinder());
        $this->assertQueryContains('HAVING count\(author_id\) = :c_0', $sql);

        $query->resetClause('having')->having(function ($e) {
    ...
    public function testBind(): void
    {
        ...
        $this->assertCount(2, $query->getValueBinder());
        $this->assertArrayHasKey(':foo', $query->getValueBinder()->bindings());
        $this->assertArrayHasKey(':bar', $query->getValueBinder()->bindings());
    }
    ...
    public function testCloneWithExpression(): void
    {
        ...
        $query
            ...
            ->with(function (CommonTableExpression $cte, AbstractQuery $query) {
                return $cte
                    ->name('cte2')
                    ->query($query)
                ;
